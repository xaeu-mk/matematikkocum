...
async function dashboard(db,user){
 const count=(sql,bind=[])=>db.prepare(sql).bind(...bind).first().then(r=>Number(r?.n||0))
 let stats={students:0,teachers:0,parents:0,courses:0,assignments:0,exams:0,messages:0,unreadMessages:0}
 if(user.role==='admin'){
  const [students,teachers,parents,courses,assignments,exams,messages,unreadMessages]=await Promise.all([
   count("SELECT COUNT(*) n FROM users WHERE role='student' AND is_active=1"),count("SELECT COUNT(*) n FROM users WHERE role='teacher' AND is_active=1"),count("SELECT COUNT(*) n FROM users WHERE role='parent' AND is_active=1"),count('SELECT COUNT(*) n FROM courses'),count('SELECT COUNT(*) n FROM assignments'),count('SELECT COUNT(*) n FROM exams'),count('SELECT COUNT(*) n FROM messages'),count('SELECT COUNT(*) n FROM messages WHERE read_at IS NULL')])
  stats={students,teachers,parents,courses,assignments,exams,messages,unreadMessages}
 }else if(user.role==='teacher'){
  const [courses,assignments,exams,messages,unreadMessages,students]=await Promise.all([count('SELECT COUNT(*) n FROM courses WHERE teacher_id=?',[user.id]),count('SELECT COUNT(*) n FROM assignments WHERE teacher_id=?',[user.id]),count('SELECT COUNT(*) n FROM exams WHERE teacher_id=?',[user.id]),count('SELECT COUNT(*) n FROM messages WHERE receiver_id=?',[user.id]),count('SELECT COUNT(*) n FROM messages WHERE receiver_id=? AND read_at IS NULL',[user.id]),count('SELECT COUNT(*) n FROM teacher_student_links WHERE teacher_id=?',[user.id])])
  stats={...stats,courses,assignments,exams,messages,unreadMessages,students}
 }else if(user.role==='student'){
  const [courses,assignments,exams,messages,unreadMessages]=await Promise.all([count('SELECT COUNT(*) n FROM courses c JOIN teacher_student_links l ON l.teacher_id=c.teacher_id WHERE l.student_id=?',[user.id]),count("SELECT COUNT(*) n FROM assignment_students WHERE student_id=? AND status!='completed'",[user.id]),count('SELECT COUNT(*) n FROM exam_results WHERE student_id=?',[user.id]),count('SELECT COUNT(*) n FROM messages WHERE receiver_id=?',[user.id]),count('SELECT COUNT(*) n FROM messages WHERE receiver_id=? AND read_at IS NULL',[user.id])])
  stats={...stats,courses,assignments,exams,messages,unreadMessages}
 }else{
  const [students,messages,unreadMessages]=await Promise.all([count('SELECT COUNT(*) n FROM parent_student_links WHERE parent_id=?',[user.id]),count('SELECT COUNT(*) n FROM messages WHERE receiver_id=?',[user.id]),count('SELECT COUNT(*) n FROM messages WHERE receiver_id=? AND read_at IS NULL',[user.id])])
  stats={...stats,students,messages,unreadMessages}
 }
 const [activity,events,goals,progress]=await Promise.all([
  db.prepare('SELECT a.action,a.created_at,u.username,u.full_name FROM audit_logs a LEFT JOIN users u ON u.id=a.user_id ORDER BY a.created_at DESC LIMIT 8').all(),
  db.prepare('SELECT id,title,start_at,end_at,type,description FROM calendar_events WHERE user_id=? ORDER BY start_at ASC LIMIT 8').bind(user.id).all(),
  db.prepare('SELECT * FROM coaching_plans WHERE student_id=? OR coach_id=? ORDER BY updated_at DESC LIMIT 6').bind(user.id,user.id).all(),
  db.prepare('SELECT metric,value,target,period,created_at FROM progress_entries WHERE student_id=? ORDER BY created_at DESC LIMIT 12').bind(user.id).all()
 ])
 return {stats,activity:activity.results||[],events:events.results||[],goals:goals.results||[],progress:progress.results||[]}
}
...
async function createResource(db,user,resource,body){
 ...
 if(resource==='courses'){if(!['admin','teacher'].includes(user.role))return null;await db.prepare(`INSERT INTO courses(id,title,description,subject,teacher_id,is_active,created_at) VALUES(?,?,?,?,?,?,?)`).bind(id,safe(body.title),safe(body.description),safe(body.subject),user.role==='teacher'?user.id:(body.teacherId||null),1,created).run();return{id}}
 if(resource==='assignments'){if(!['admin','teacher'].includes(user.role))return null;await db.prepare(`INSERT INTO assignments(id,title,description,course_id,teacher_id,due_at,created_at) VALUES(?,?,?,?,?,?,?)`).bind(id,safe(body.title),safe(body.description),body.courseId||null,user.role==='teacher'?user.id:(body.teacherId||null),body.dueAt||null,created).run();if(Array.isArray(body.studentIds))for(const sid of body.studentIds)await db.prepare(`INSERT OR IGNORE INTO assignment_students(assignment_id,student_id) VALUES(?,?)`).bind(id,sid).run();return{id}}
 if(resource==='calendar'){await db.prepare(`INSERT INTO calendar_events(id,user_id,title,description,start_at,end_at,type,created_by,created_at) VALUES(?,?,?,?,?,?,?,?,?)`).bind(id,user.id,safe(body.title),safe(body.description),body.startAt,body.endAt||body.startAt,body.type||'study',user.id,created).run();return{id}}
 if(resource==='messages'){if(!body.receiverId||!body.body)return null;await db.prepare(`INSERT INTO messages(id,sender_id,receiver_id,body,created_at) VALUES(?,?,?,?,?)`).bind(id,user.id,body.receiverId,safe(body.body),created).run();return{id}}
 if(resource==='coaching'){if(!['admin','teacher'].includes(user.role))return null;await db.prepare(`INSERT INTO coaching_plans(id,student_id,coach_id,goal,status,notes,next_review_at,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?)`).bind(id,body.studentId,user.id,safe(body.goal),'active',safe(body.notes),body.nextReviewAt||null,created,created).run();return{id}}
 if(resource==='evaluations'){if(!['admin','teacher'].includes(user.role))return null;await db.prepare(`INSERT INTO evaluations(id,student_id,teacher_id,title,score,feedback,created_at) VALUES(?,?,?,?,?,?,?)`).bind(id,body.studentId,user.id,safe(body.title),Number(body.score)||0,safe(body.feedback),created).run();return{id}}
 if(resource==='progress'){const studentId=user.role==='student'?user.id:body.studentId;if(!studentId)return null;await db.prepare(`INSERT INTO progress_entries(id,student_id,metric,value,target,period,created_at) VALUES(?,?,?,?,?,?,?)`).bind(id,studentId,safe(body.metric),Number(body.value)||0,body.target==null?null:Number(body.target),safe(body.period),created).run();return{id}}
...
export async function onRequest(context){
 ...
 try{
  ...
  if(context.request.method==='GET'){
   ...
   if(resource==='settings'){let settings=await db.prepare('SELECT * FROM user_settings WHERE user_id=?').bind(user.id).first();if(!settings){await db.prepare(`INSERT INTO user_settings(user_id) VALUES(?)`).bind(user.id).run();settings=await db.prepare('SELECT * FROM user_settings WHERE user_id=?').bind(user.id).first()}return json({ok:true,settings})}
   return json({ok:true,items:await resourceList(db,user,resource)})
  }
  const body=await context.request.json().catch(()=>({}))
  if(context.request.method==='POST'){const result=await createResource(db,user,resource,body);if(!result)return json({ok:false,error:'Bu işlem için yetkiniz yok veya eksik bilgi var.'},403);await db.prepare('INSERT INTO audit_logs(id,user_id,action,target_id,metadata,created_at) VALUES(?,?,?,?,?,?)').bind(randomId(),user.id,`create_${resource}`,result.id,JSON.stringify(body),now()).run();return json({ok:true,...result},201)}
  return json({ok:false,error:'Method desteklenmiyor.'},405)
 }catch(error){console.error('APP_ERROR',error);return json({ok:false,error:error?.message||'Beklenmeyen sunucu hatası.'},500)}
}
