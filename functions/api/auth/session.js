export async function onRequestGet(context) {
  ...
  if (!db) return Response.json({ authenticated: false, user: null, configured: false }, { headers })

  const cookie = context.request.headers.get('Cookie') || ''
  const match = cookie.match(/(?:^|;\s*)mk_session=([^;]+)/)
  if (!match) return Response.json({ authenticated: false, user: null, configured: true }, { headers })
  ...
  if (!session || !session.is_active || new Date(session.expires_at) <= new Date()) {
    return new Response(JSON.stringify({ authenticated: false, user: null, configured: true }), {
      status: 200,
      headers: { ...headers, 'Content-Type': 'application/json', 'Set-Cookie': 'mk_session=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0' },
    })
  }
